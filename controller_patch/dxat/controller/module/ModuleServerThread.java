...
public class ModuleServerThread implements Runnable {
    ...
    private class AppServer implements Runnable {
        ...
        public String getKey() {
            return socket.getInetAddress().getHostAddress().toString() + ":" + socket.getPort();
        }
        ...
        public void sendControllerEvent(ControllerEvent controllerEvent) {
            ...
            try {
                ...
                System.out.println("[Sending Event to key '" +
                        getKey() + "'] " + controllerEvent.getEvent());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        ...
        public void run() {
            while (!socket.isClosed() && socket.isConnected() && !socket.isInputShutdown() && !socket.isOutputShutdown()) {
                try {
                    // Read line command
                    String newLine = new String(reader.readLine());
            ...
            System.out.println("[REMOVING CONNECTION] AppServer with key '" + getKey() + "'");
        }
        ...
        private void processRequest(ServerRequest serverRequest) {
            ...
            try {
                if (serverRequest.getRequest().equals(IServerRequests.TERMINALS_REQUEST)) {
                    ...
                    DeployedFlowCollection deployedFlowCollection = DxatAppModule.getInstance().getFlowPusherManager().getDeployedFlows();
                    controllerEvent.setEvent(new Gson().toJson(deployedFlowCollection));
    ...
    public void broadcastControllerEvent(ControllerEvent controllerEvent) {
        List<AppServer> appServerList = new ArrayList<AppServer>(serversMap.values());
        ...
        while (true) {
            try {
                ...
                if (!serversMap.containsKey(appServer.getKey())) {
                    ...
                    System.out.println("[NEW CONNECTION] AppServer with key '" + appServer.getKey() + "'");
                } else {
                    socket.close();
                    System.out.println("[NEW CONNECTION REFUSED] Already exist key: '" + appServer.getKey() + "'");
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
